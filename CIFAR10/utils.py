...
def post_train(model, images, train_loader, train_loaders_by_class, args):
    ...
    with torch.enable_grad():
        ...
        for _ in range(args.pt_iter):
            ...
            # adv_input = data + (torch.randint(0, 2, size=(len(data),)) - 0.5).to(device) * 2 * neighbour_delta.detach()
            # adv_input = data + (torch.rand_like(data) - 0.5).to(device) * 2 * neighbour_delta.detach()
            adv_input = data + -1 * torch.rand_like(data).to(device) * neighbour_delta.detach()
            # adv_input = data + (torch.randint(0, 2, size=()) - 0.5).to(device) * 2 * neighbour_delta.detach()
            # adv_input = data + -0.5 * neighbour_delta.detach()
            # directed_delta = torch.vstack([torch.ones_like(original_data).to(device) * neighbour_delta.detach(),
            #                                 torch.ones_like(neighbour_data).to(device) * -1 * neighbour_delta.detach()])
