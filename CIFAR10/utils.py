...
def evaluate_pgd_post(test_loader, train_loaders_by_class, model, attack_iters, restarts):
    epsilon = (8 / 255.) / std
    alpha = (2 / 255.) / std
    pgd_loss = 0
    pgd_acc = 0
    ...
    for i, (X, y) in enumerate(test_loader):
        ...
        calculate_loss_surface(model, [model, post_model], X, y, attack_func=attack_pgd)
